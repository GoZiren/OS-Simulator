...
std::string Directory::ls_r(bool detail) const
{
	std::string str = "";
	if (detail)
	{
		for (auto c : current->child)
		{
			...
			{
				Folder* ptr = dynamic_cast<Folder*>(c);
				str = str + "\t count: " + std::to_string(ptr->count()) + "\n";
			}
			else
			{
				str = str + "\n";
			...
			str = str + "\t last mod: "
				+ " by userId " + std::to_string(c->access.lastEditor) + "\t"
				+ " at " + c->timetostr(c->access.lastModification)
				+ "\t last acc: "
				+ " by userId " + std::to_string(c->access.accessby) + "\t"
				+ " at " + c->timetostr(c->access.lastAccess) + "\n";
		}
		std::cout << "\n";
	}
...
std::string Directory::lsMount_r() const
{
	std::string str = "";
	for (auto c : mountPoints)
	{
		str = str +  "Point" + std::to_string(-(c.first)) + " => " + c.second->path + "\n";
	}
	return str;
}
...
bool Directory::cat(std::string name,std::string &result) const
{
	std::ifstream fi;
	...
	if (!current->find(name, vec, false, true, 1))
	{
        ...
        result += "cat: cannot read " + name + ": No such file";
		return false;
	}
	...
	if (!fi.is_open())
	{
        ...
        result += "Error when open the file";
		return false;
	}
...
bool Directory::open(std::string name,std::string &result)
{
	std::vector<FCB*> vec;
	if (!current->find(name, vec, false, true, 1))
	{
		std::cout << "open: cannot open " << name << ": No such file" << std::endl;
        result += "open: cannot open " + name + ": No such file";
		return false;
	}
	...
	for (auto c : openFiles)
	{
		if (c->nodeId == ptr->nodeId)
		{
            ...
            result += "open: cannot open " + name + ":File is already opened";
			return false;
		}
	}
...
std::string Directory::lsOpen_r() const
{
	std::string str = "";
	if (openFiles.empty())
	{
		str = str + "lsOpen: None file is open" + "\n";
		return str;
	}
	for (auto c : openFiles)
	{
		str = str + "name: " + c->name + "\tID: " + std::to_string(c->nodeId) + "\n";
	}
}
...
bool Directory::put(std::string str, bool end, bool replace)
{
	...
	if (currentObj->location == 0)
	{
		...
		{
			int old_num = (currentObj->size() / unit_block_size) + 1;
			int add_size = (currentObj->size() + str.size()) - old_num *  unit_block_size;
			int i = 0;
